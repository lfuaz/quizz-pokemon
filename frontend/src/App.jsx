...
function App() {
  ...
  return pokemon.descriptions ? (
    <div className="App">
      ...
      {isPokemonFound && catchingprocess === "caught" ? <DrawPokeball /> : null}

      {isPokemonFound ? (
        <button
          className={
            statusPokemon != "escaped" ? "action catch" : "action catch failed"
          }
              ...
              {statusPokemon === "caught" ? (
                <Countdown
                  ...
                  renderer={(countdownProps) =>
                    renderer({ ...countdownProps, next: handlePokemonChange })
                  }
                />
              ) : null}
